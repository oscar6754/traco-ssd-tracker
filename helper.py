...
def cdist(XA, XB, metric='euclidean', *args, **kwargs):
    ...
    Parameters:
    - XA : array_like
        An array containing M vectors of dimension N.
    - XB : array_like
        An array containing L vectors of dimension N.
    - metric : str or callable, optional
        The distance metric to use. It must be a string or a callable.
        The callable should take two arrays from XA and XB as input and return
        a float.
    - *args : additional positional arguments
        Any additional positional arguments supported by the distance metric
        function.
    - **kwargs : additional keyword arguments
        Any additional keyword arguments supported by the distance metric
        function.
    ...
    if metric == 'euclidean':
        return np.sqrt(((XA[:, None] - XB) ** 2).sum(axis=-1))
